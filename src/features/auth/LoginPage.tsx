      ...
      >
        <motion.div variants={listItemVariants} className="flex flex-col items-center">
          ...
          <h1 className="mt-5 text-3xl font-bold tracking-tight text-ink">
            VShape<span className="text-info-ink">100</span>
          </h1>
          <p className="mt-1.5 text-sm text-ink-3">Build your foundation.</p>
        </motion.div>
        ...
        <motion.div variants={listItemVariants} className="mt-9">
          <motion.button
            ...
            className="flex w-full items-center justify-center gap-3 rounded-control bg-ink px-5 py-3.5 text-[15px] font-bold text-ink shadow-card transition-opacity duration-fast disabled:opacity-70"
          >
            {isRedirecting ? (
              <>
        ...
        {errorMessage && (
          <motion.p
            ...
            className="mt-4 flex items-start gap-2 rounded-control border border-danger-ink/30 bg-danger-ink/10 px-3.5 py-3 text-[13px] font-semibold text-danger-ink"
          >
        ...
        */}
          ...
          <label className="flex cursor-pointer items-start gap-3 rounded-control px-1 py-1">
            <span className="relative mt-0.5 shrink-0">
              <input
                ...
                className="peer block size-[18px] cursor-pointer appearance-none rounded-[6px] border-2 border-line-strong bg-surface transition-colors duration-fast checked:border-accent-edge checked:bg-accent"
              />
              <Check
                aria-hidden="true"
                className="pointer-events-none absolute inset-0 m-auto size-3 text-ink opacity-0 transition-opacity duration-fast peer-checked:opacity-100"
                strokeWidth={3.5}
              />
            </span>
            <span className="leading-tight">
              <span className="block text-sm font-bold text-ink-2">Trust this device</span>
        ...
        <motion.p
          ...
          className="mt-10 text-center text-[11px] font-semibold uppercase tracking-[0.09em] text-ink-3"
        >
          Private · Personal
        </motion.p>
      </motion.div>
